...
"-em 0 2 1 0.680304",
"-ej 1.89327 2 1",
"-en 1.89327 1 26.2245",
"-en 21.58982 1 1",
sep=" ")),what="character",sep="\n")->junk

msparse.2pops(junk,n1=96,n2=96,nsites=5000)->test
test<-cbind(test,test[,4]-test[,1])
test<-cbind(test,test[,6]-test[,3])
test[,8]<-(test[,8]-mean(test[,8]))/sd(test[,8])
test[,9]<-(test[,9]-mean(test[,9]))/sd(test[,9])

write.table(test,"testBINBH.txt")
