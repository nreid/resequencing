...
nsites<-5000
the<-0.0028974710456464423*nsites
...
msparse.2pops(junk,n1=96,n2=96,nsites=5000)->testSHNYC
testSHNYC<-cbind(testSHNYC,testSHNYC[,4]-testSHNYC[,1])
testSHNYC<-cbind(testSHNYC,testSHNYC[,6]-testSHNYC[,3])
testSHNYC[,8]<-(testSHNYC[,8]-mean(testSHNYC[,8]))/sd(testSHNYC[,8])
testSHNYC[,9]<-(testSHNYC[,9]-mean(testSHNYC[,9]))/sd(testSHNYC[,9])

write.table(testSHNYC,"testSHNYC.txt")
